...
public class MainGameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rMan.Raycast(Input.GetTouch(0).position, rayHits) && plane == null)
        {
            ...
            GameObject centerTarget = Instantiate(Target, plane.center + new Vector3(0, .1f, 0), Quaternion.identity);
            centerTarget.transform.SetParent(plane.transform);

            for (int i = 1; i < plane.boundary.Length - 1; i += plane.boundary.Length/4)
            {
                GameObject randomTarget = Instantiate(Target, plane.center + new Vector3(((plane.boundary[i].x) / 2), .1f, ((plane.boundary[i].y) / 2)), Quaternion.identity);
